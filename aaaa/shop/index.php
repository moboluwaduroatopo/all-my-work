...
<?php				
				...
				// {
				// 	$filter = $_POST['filter'];
				// 	$result = mysqli_query($bd, "SELECT * FROM products where product_name like '%$filter%' or Description like '%$filter%' or Category like '%$filter%'");
				...
				$prodID = $row["product_id"];	
					...
					<a href="product-details.php?prodid='.$prodID.'" rel="bookmark" title="'.$row['product_name'].'"><img src="reservation/img/products/'.$row['imgUrl'].'" alt="'.$row['product_name'].'" title="'.$row['product_name'].'" width="150" height="150" /></a>
                    </a>
