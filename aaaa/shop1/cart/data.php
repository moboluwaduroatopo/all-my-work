...
class Shopping {
    ...
    function countorder(){
        $q = "select * from ad10cakstore.order where status='unconfirmed'";
        $result = mysqli_query($bd, $q);
        echo mysqli_num_rows($result);
    }
    function countproducts(){
        $q = "select * from ad10cakstore.product_tb";
        $result = mysqli_query($bd, $q);
        echo mysqli_num_rows($result);
    }
    function countcategory(){
        $q = "select * from ad10cakstore.type_tb";
        $result = mysqli_query($bd, $q);
        echo mysqli_num_rows($result);
    }
    function checkout(){
        ...
        echo $q = "INSERT INTO ad10cakstore.order VALUES (NULL, '$fullname', '$contact', '$address', '$email', '$item', '$amount', 'unconfirmed', '$date', '')";
    ...
    function verify(){
        ...
        $q = "SELECT * from ad10cakstore.user where username='$username' and password='$password'";
        $result = mysqli_query($bd, $q);
        $_SESSION['login']='yes';
        echo mysqli_num_rows($result);
    }
