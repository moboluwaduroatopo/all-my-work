...
class DBOperation
{
	...
	public function addCategory($parent,$cat){
		$pre_stmt = $this->con->prepare("INSERT INTO `categories`( `category_name`, `cimage`)
		 VALUES (?,?,?)");
		$status = 1;
		$pre_stmt->bind_param("isi",$cat,$cimage);
	...
	public function addProduct($cid,$pro_name,$price,$stock,$date,$image){
		$pre_stmt = $this->con->prepare("INSERT INTO `products`
			(`cid`, `product_name`, `product_price`,
			 `product_stock`, `added_date`, `pimage`)
			 VALUES (?,?,?,?,?,?,?)");
		$status = 1;
		$pre_stmt->bind_param("iisdisi",$cid,$pro_name,$price,$stock,$date,$image);
		$result = $pre_stmt->execute() or die($this->con->error);
		if ($result) {
