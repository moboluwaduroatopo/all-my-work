...
class Manage
{
	...
	public function manageRecordWithPagination($table,$pno){
		...
		if ($table == "categories") {
			$sql = "SELECT p.cid,p.category_name as category, c.category_name as category, p.image FROM categories p LEFT JOIN categories c ON p.image=c.cid ".$a["limit"];
		}else if($table == "products"){
			$sql = "SELECT p.pid,p.product_name,c.category_name,p.product_price,p.product_stock,p.added_date,p.p_status FROM products p,brands b,categories c WHERE p.bid = b.bid AND p.cid = c.cid ".$a["limit"];
		}else{
