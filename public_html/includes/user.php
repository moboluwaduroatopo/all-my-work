...
class User
{
	...
	public function createUserAccount($sname,$mname,$lname,$email,$username,$password,$ppt){
		...
		if ($this->emailExists($email)) {
			...
			$date = date("Y-m-d");
			$notes = "";
			$pre_stmt = $this->con->prepare("INSERT INTO `admin(surname,middlename,lastname,email,username, password,passport)VALUES('$sname','$mname','$lname','$email','$username','$password','$ppt')");
